...
asking**. When the conversation has grown past half of the context window, the summary ends by
suggesting `/compact` or a fresh session: every turn re-reads the whole context, and the batch
files already hold everything a resume needs.
