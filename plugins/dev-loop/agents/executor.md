...
from a test — through the public surface, with a seam for whatever leaves the process. Adding a
seam later is a production change, and it costs a round trip through the user. An entry that
turns out wrong or missing goes in `deviations`; the list is not yours to rewrite.
